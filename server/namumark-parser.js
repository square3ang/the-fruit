...
import fs from 'fs';
//import diff from './cemerick-jsdifflib.js';
...
function alertBalloon(content, type = 'danger', dismissible = true, classes = '', noh, id) {
	return `<script>loadAlert('${content}', '${type}', ${dismissible}, '${classes}');</script>`;
}
...
function fetchNamespaces() {
	return ['문서', '틀', '분류', '파일', '사용자', '특수기능', "정사각위키", '토론', '휴지통', '투표'];
}
...
export async function markdown(req, content, discussion = 0, title = '', flags = '', root = '') {
	...
	function filterCSS(style) {
		var ret = '';
		var asdf = style.split(';');
		for(var asdfasdf of asdf) {
			var prop  = asdfasdf.split(':')[0]?.trim();
			var value = asdfasdf.split(':')[1]?.trim();
